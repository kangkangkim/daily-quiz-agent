...
- **实时排行榜** 🏆：进门第一眼就是它——右侧栏顶部金色高亮卡片，全员排名（累计答对 > 连续答对 > 今日是否答对 > 参与天数）；**任何人作答，所有打开的页面 ≤0.5s 内自动刷新**（SSE 推送 `GET /api/leaderboard/stream`，断线自动降级轮询）；前三名领奖台 + 第 4 名起列表，名次变动行会闪烁提示，自己那行高亮
- **元气教室界面** 🎨：暖米底 + 珊瑚/琥珀/青绿活力配色，Hero 期号带实时"今日作答/答对"人数，答对时撒花 🎉；三栏布局——左：往期回顾侧栏 / 中：题目+康康答疑+统计+知识图谱 / 右：排行榜；**康康答疑紧跟题目正下方，提交答案后自动滚入视野**，答完看讲解一气呵成（中屏收两栏、往期回顾变顶部横向条带，窄屏单栏题目优先）
- **聊天 Markdown 渲染**：康康回复中的表格/列表/标题/加粗/行内代码安全渲染（先整体 HTML 转义再做结构转换，防 XSS）
- **往期回顾** 📚：左侧常驻侧栏（中窄屏为顶部横向条带），`GET /api/history`；点任意一期弹出详情层，可回顾题干/选项（正确答案高亮）/答案/考察要点/解析；今日题不进历史，防剧透
- **知识图谱** 🧠：`GET /api/graph` 由题库动态生成——科目为中心节点、每道题为叶子节点，沿学习路径生长；青绿✓已解锁 / 珊瑚红光晕今日 / 灰虚线待解锁，连线有流光动画，点击叶子可跳转对应期回顾（Canvas 2D，respecting `prefers-reduced-motion`）
- **出题管理页** `/admin.html`：可视化加题/改题，或用 CLI 脚本
- **SSE 流式输出**：康康小老师的回复边生成边显示
- 测试深链：`/#name=某某` 预设答题人；`/#history=1` 滚动到往期回顾；`/#open=1` 或 `/#open=2026-08-28` 自动弹出最近一期/指定日期的回顾详情
...
13. 知识图谱：`/api/graph` 按科目分组，每道题带 past/today/future 状态与真实期号，且存在今日节点
