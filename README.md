...
每天打开 → 做一道题 → 小老师 Agent **判对判错** → 围绕题目**答疑讲解**。

## 功能

- **每日一题**：按日期（默认 Asia/Shanghai）出题，题目含题干/选项/标准答案/要点 keyPoints/解析，存在 `questions/<日期>.json`
- **Agent 判题**：小老师 Agent 通过工具读取标准答案后判定；**选择题由服务端确定性校正**（字母精确匹配），不依赖 LLM 心情
- **答疑小老师**：提交答案前只给提示不剧透；答错引导重试（错 2 次或主动放弃才公布答案）；答对后给完整讲解
- **答题统计**：今日作答次数、累计答对天数、连续答对天数
- **排行榜** 🏆：全员排名（累计答对 > 连续答对 > 今日是否答对 > 参与天数），前 3 名奖牌、自己那行高亮，不在前 10 显示"我的名次"；`GET /api/leaderboard`
- **出题管理页** `/admin.html`：可视化加题/改题，或用 CLI 脚本
- **SSE 流式输出**：小老师的回复边生成边显示
...
├── agent/teacher.js     # 小老师 Agent（系统提示词 + 判题/读题工具 + 确定性校正）
